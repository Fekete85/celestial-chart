...
`harness/compare.mjs` — két referencia-háló vetítésenkénti diffje: max /
...
átvettük a v3-as forrásból, és a `test/projections.test.mjs` a **pinelt v3-as
...
`harness/reference-d3v3.json` (pinelt upstream) vs `harness/reference-d3v7.json`
...
# http://127.0.0.1:8877/harness/reference.html      (pinelt v3)
# http://127.0.0.1:8877/harness/reference-new.html   (migrált build)
...
felületről semmit nem mond. Külön megnézve (`demo/full.html`) négy hiba jött elő:
...
A `harness/visual.html` és a `harness/visual-new.html` ugyanazt a nézetet
rajzolja a régi, illetve az új kóddal; a `harness/image-diff.html` pixelenként veti
össze a két PNG-t.
...
- `demo/module.html`: `<script type="module">`-lal, globális `Celestial` és `d3`
  nélkül — 5044 csillag, 0 hiba.
- `demo/full.html`: a teljes felület globális alakban — zoom, húzásos forgatás,
...
- `demo/two-maps.html`: két térkép egy oldalon, külön vetítéssel és
...
- `demo/full.html`: a teljes régi felület — zoom, húzásos forgatás, négy
...
A `test/projections.test.mjs` hat vetítést mér a pinelt v3-as build 162 pontos
...
Ez most állandó ellenőrzés a füstpróbában (`harness/no-container.html`).
...
`demo/two-forms.html`: két interaktív térkép, mindegyik saját űrlappal. Az A
...
**mérjük, ne higgyük**. A `test/types.test.mjs` négy állítást tesz:
