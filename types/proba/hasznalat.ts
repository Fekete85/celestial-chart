...
import Celestial, { Egbolt, Config, Vetites, Kozeppont } from "../celestial.js";
...
const a: Egbolt = new Egbolt({ container: "map-a", projection: "orthographic" }, { onallo: true });
...
b.resize({ width: 600 });
const m = a.metrics();
const szeles: number = m.width + m.margin[0];

// a vetítés használata
const pont = a.mapProjection([180, 55]);
if (pont) { const x: number = pont[0]; console.log(x); }
const lathato: 0 | 1 = a.clip([180, 55]);

// űrlap példányonként
a.urlap.fldEnable("horizon-show", true);
b.urlap.setCenter([0, 0, 0], "equatorial");
...
const vissza = Celestial.horizontal.inverse(new Date(), [hor[0], hor[1]], [47.5, 19.2]);
console.log(nagyitas, vet, szeles, lathato, oraszog, vissza, a === b);
